...
void main() {
  group('CLI command registry', () {
    test('every top-level command has a non-empty description', () {
      for (final command in agentMemoryCommands) {
        expect(
          command.name,
          isNotEmpty,
          reason: 'Command name must not be empty',
        );
        expect(
    ...
    test('every memory subcommand has a non-empty description', () {
      ...
      for (final sub in memory.subcommands) {
        expect(
          sub.name,
          isNotEmpty,
          reason: 'Subcommand name must not be empty',
        );
        expect(
          sub.hasDescription,
          isTrue,
          reason:
              'Memory subcommand "${sub.name}" is missing a description. '
    ...
    test(
      '--help output contains every top-level command and its description',
      () {
        final parser = buildRootParser();
    ...
    test(
      '--help output contains every memory subcommand and its description',
      () {
        final parser = buildRootParser();
        final buffer = StringBuffer();
        printAgentMemoryUsage(parser: parser, sink: buffer);
        ...
        final memory = agentMemoryCommands.firstWhere(
          (c) => c.name == 'memory',
        );
        for (final sub in memory.subcommands) {
          expect(
            ...
            contains(sub.name),
            reason:
                'Help output should mention memory subcommand "${sub.name}"',
          );
          expect(
            output,
            contains(sub.description),
            reason:
                'Help output should describe memory subcommand "${sub.name}"',
          );
        }
      },
    );
