...
final List<CliCommand> agentMemoryCommands = [
  CliCommand(
    name: 'process',
    description:
        'Analyze input text/images and build or update the knowledge base.',
    buildParser: _processCommandParser,
  ),
  CliCommand(
    name: 'regenerate',
    description:
        'Regenerate structure files (topics, areas, people, stats) from existing Q/A/N records.',
    buildParser: _regenerateCommandParser,
  ),
  CliCommand(
    name: 'stats',
    description:
        'Regenerate statistics, indexes, and the knowledge-base INDEX.md.',
  ...
  CliCommand(
    name: 'search',
    description:
        'Search the knowledge base using natural language (AI generates tags from the query).',
    buildParser: _searchCommandParser,
  ),
  CliCommand(
    name: 'skill',
    description:
        'Show a structured skill/cheat-sheet for LLM agents (what the framework does and how to call it).',
    buildParser: _skillCommandParser,
  ),
  CliCommand(
    name: 'memory',
    description:
        'Agent memory CRUD: add, ask, list, delete, rank, and update records.',
    ...
    subcommands: [
      ...
      CliCommand(
        name: 'list',
        description:
            'List memory records, optionally filtered by type or tags.',
      ...
      CliCommand(
        name: 'rank',
        description:
            'Show top-ranked memory records by access count, importance, or recency.',
        buildParser: _memoryRankCommandParser,
      ),
      CliCommand(
        name: 'update',
        description:
            'Update text, tags, or importance of an existing memory record.',
        buildParser: _memoryUpdateCommandParser,
      ),
      CliCommand(
        name: 'consolidate',
        description:
            'Consolidate memory records into a high-level MEMORY.md summary and skill cards.',
      ...
      CliCommand(
        name: 'promote',
        description:
            'Promote a note to a higher memory level (1 raw → 2 consolidated → 3 concept).',
...
ArgParser _processCommandParser() => ArgParser()
  ...
  ..addOption(
    'source',
    abbr: 's',
    help: 'Source name (defaults to input file name)',
  )
  ..addOption(
    'provider',
    defaultsTo: 'openai',
    help: 'LLM provider: openai, openrouter or ollama',
  )
  ..addOption(
    'api-key',
    help: 'API key (defaults to OPENAI_API_KEY / OPENROUTER_API_KEY env)',
  )
  ..addOption('base-url', help: 'Base URL for the chat completions endpoint')
  ..addOption(
    'model',
    help: 'Model name (defaults to OPENAI_MODEL / OPENROUTER_MODEL env)',
  )
  ...
  ..addOption(
    'mode',
    defaultsTo: 'full',
    help: 'Processing mode: full, process-only, aggregate-only',
  )
  ..addFlag('clean', help: 'Clean output directory before processing')
  ..addFlag('verbose', abbr: 'v', help: 'Verbose output')
  ..addOption(
    'analysis-instructions',
    help: 'Extra instructions for the analysis agent',
  )
  ..addOption(
    'aggregation-instructions',
    help: 'Extra instructions for the aggregation agent',
  )
  ..addOption(
    'qa-mapping-instructions',
    help: 'Extra instructions for the Q&A mapping agent',
  );
...
ArgParser _searchTagsCommandParser() => ArgParser()
  ...
  ..addOption(
    'type',
    help: 'Comma-separated entity types: question,answer,note',
  )
...
ArgParser _memoryAskCommandParser() => ArgParser()
  ...
  ..addOption(
    'as-of',
    help: 'Only records known at this ISO date/time (e.g., 2025-03-15)',
  )
  ..addOption(
    'provider',
    defaultsTo: 'openai',
...
ArgParser _memoryListCommandParser() => ArgParser()
  ...
  ..addOption(
    'sort',
    defaultsTo: 'lastAccessed',
    help: 'Sort by: lastAccessed, accessCount, importance',
  )
...
ArgParser _memoryRankCommandParser() => ArgParser()
  ...
  ..addOption(
    'sort',
    defaultsTo: 'accessCount',
    help: 'Sort by: accessCount, importance, lastAccessed',
  )
  ..addFlag('json', help: 'Output as JSON');
...
ArgParser _memoryUpdateCommandParser() => ArgParser()
  ...
  ..addOption(
    'memory-type',
    help:
        'For notes: fact, event, observation, belief, decision, rule, experience',
  )
...
ArgParser _memoryAddCommandParser() => ArgParser()
  ...
  ..addOption(
    'type',
    abbr: 't',
    help: 'Record type: question, answer, note',
    mandatory: true,
  )
  ...
  ..addOption(
    'answers-question',
    help: 'For answers: id of the question being answered',
  )
...
ArgParser _memoryPromoteCommandParser() => ArgParser()
  ...
  ..addOption(
    'level',
    abbr: 'l',
    help: 'Target level: 1 raw, 2 consolidated, 3 concept',
    mandatory: true,
  );
...
ArgParser _memoryConsolidateCommandParser() => ArgParser()
  ...
  ..addOption(
    'instructions',
    help: 'Extra instructions for the consolidation agent',
  );
