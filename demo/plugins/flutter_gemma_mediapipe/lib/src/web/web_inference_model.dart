...
class WebModelSession extends InferenceModelSession {
  ...
  Future<String> getResponse() async {
    ...
    try {
      ...
      // Always log the exact prompt to the browser console so deployed builds
      // can be debugged without rebuilding in profile mode.
      if (promptArray is JSString) {
        print('[MediaPipe Web] prompt string: ${promptArray.toDart}');
      } else {
        print('[MediaPipe Web] prompt array elements:');
        final arr = promptArray as JSObject;
        final len = (arr.getProperty('length'.toJS) as JSNumber).toDartInt;
        for (var i = 0; i < len; i++) {
          final item = arr.getProperty(i.toJS);
          if (item.typeofEquals('string')) {
            print('  [$i] string: ${(item as JSString).toDart}');
          } else {
            print('  [$i] object: ${(item as JSObject).toString()}');
          }
        }
      }
      ...
      print(
        '[MediaPipe Web] response length: ${response.length}, preview: ${response.isEmpty ? "(empty)" : response.substring(0, math.min(200, response.length))}',
      );

      // Don't add response back to promptParts - that's handled by InferenceChat
      return response;
    } catch (e, stackTrace) {
      print('[MediaPipe Web] getResponse exception: $e');
      print('[MediaPipe Web] stack trace: $stackTrace');
      if (kDebugMode) {
  ...
  Stream<String> getResponseAsync() {
    ...
    try {
      ...
      print('[MediaPipe Web] async prompt type: ${promptArray.runtimeType}');
      if (promptArray is JSString) {
        print('[MediaPipe Web] async prompt string: ${promptArray.toDart}');
      } else {
        print('[MediaPipe Web] async prompt array elements:');
        final arr = promptArray as JSObject;
        final len = (arr.getProperty('length'.toJS) as JSNumber).toDartInt;
        for (var i = 0; i < len; i++) {
          final item = arr.getProperty(i.toJS);
